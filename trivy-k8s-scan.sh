...
docker run --rm -v /var/lib/trivy-cache:/root/.cache/ \
  ...
  aquasec/trivy:latest image --skip-db-update --exit-code 0 --severity LOW,MEDIUM,HIGH $imageName

docker run --rm -v /var/lib/trivy-cache:/root/.cache/ \
  -e TRIVY_DB_REPOSITORY=ghcr.io/aquasecurity/trivy-db \
  -e TRIVY_JAVA_DB_REPOSITORY=ghcr.io/aquasecurity/trivy-java-db \
  aquasec/trivy:latest image --skip-db-update --exit-code 0 --severity CRITICAL $imageName
